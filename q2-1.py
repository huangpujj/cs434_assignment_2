...
def main(train_x,train_y,test_x,test_y):
        ...
        DT_print_out(root, 0)
        print("")
        print("Information gain for decision stump:")
        print(root.infoGain)
        print("")
        print("The training error:")
        print(calculate_Error(x,y, root))
        print("")
        print("The testing error:")
        print(calculate_Error(x_test,y_test, root))



def DT_print_out(node,depth):
    if node.type == 2:
        print("Y = " + str(node.value) + " (Entropy: " + str(round(node.entropy, 2)) + ")")
        return
    Print_node(node, depth, True)
    DT_print_out(node.left, depth + 1)
    Print_node(node, depth, False)
    DT_print_out(node.right, depth + 1)

def Print_node(node, depth, bool):
    message = "X_" + str(node.split_id + 1) + " "
    compare = "<" if bool else ">="
    message = message + compare + " " + str(round(node.split_value, 2)) + ": "
    message = message + "(Entropy: " + str(round(node.entropy, 2))
    message = message + ", Gain: " + str(round(node.infoGain, 2)) + ")"
    print(message)
